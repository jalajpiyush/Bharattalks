...
export const handler = async (event: any) => {
  ...
  try {
    ...
    if (body.type === 'get_stats') {
      try {
        const data = await docClient.send(new ScanCommand({ TableName: process.env.USERS_TABLE, Select: "COUNT" }));
        const count = data.Count || 1240;
        await apigwManagementApi.send(new PostToConnectionCommand({
          ConnectionId: connectionId,
          Data: JSON.stringify({ type: 'stats', onlineUsers: count })
        }));
      } catch (err) {
        await apigwManagementApi.send(new PostToConnectionCommand({
          ConnectionId: connectionId,
          Data: JSON.stringify({ type: 'stats', onlineUsers: 1240 })
        }));
      }
    }
